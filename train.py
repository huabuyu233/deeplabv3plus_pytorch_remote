...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path  = "" # 类别数不匹配，不建议直接加载21类的全模型权重，改为加载主干权重
    #---------------------------------------------------------#
    #   下采样的倍数8、16
    ...
    Init_Epoch          = 0
    Freeze_Epoch        = 50
    Freeze_batch_size   = 4   # Xception比较大，显存4G建议调小为4
    Freeze_lr           = 3e-4 # Xception建议学习率设小一点
    #----------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #----------------------------------------------------#
    UnFreeze_Epoch      = 200
    Unfreeze_batch_size = 2   # 解冻后显存要求极高，4G显卡必须设为2
    Unfreeze_lr         = 1e-5 # Xception建议学习率设小一点
    #------------------------------#
    #   数据集路径
    #------------------------------#
    ...
    #---------------------------------------------------------------------#
    # 类别不平衡问题：遥感图像中水体、道路、建筑比例不一
    # 类别顺序为: ["bare soil", "building", "pavement", "road", "vegetation", "water"]
    # 根据之前的测试结果：pavement(0.39) 和 vegetation(0.41) 最低，water(0.92) 和 road(0.80) 最高
    # 适当提高低分项的权重，降低高分项的权重，以平稳梯度更新
    cls_weights     = np.array([1.0, 1.0, 1.4, 0.8, 1.3, 0.7], np.float32)
    #------------------------------------------------------#
